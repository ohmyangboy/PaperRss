...
run_feature_tests() {
    ...
    python3 scripts/build-support.py --lane tests --temporary -- swift test --scratch-path "$PWD/.build" --filter 'AppFeatureRegressionTests|EntryPreviewImageTests|ArticlePreviewPersistenceTests|ArticleThumbnailStoreTests|TimelinePresentationTests|testArticlePreviewFromFreshRSS'
...
run_highlight_webkit_test() {
    echo -e "\n${BLUE}▶ 执行代码高亮真实 WKWebView 完成态探针...${NC}"
    ./scripts/test-highlight-webkit.sh
